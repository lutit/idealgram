...
//        }
    ...
    private void sendPreparedTextMessage(CharSequence message, boolean notify, int scheduleDate, int scheduleRepeatPeriod, long payStars, SendMessageInternalParams internalParams) {
        if (delegate != null) {
    ...
    private void applyShamalaAndSend(final CharSequence originalMessage, final boolean notify, final int scheduleDate, final int scheduleRepeatPeriod, final long payStars, final SendMessageInternalParams internalParams) {
        ...
        SHAMALA_HTTP_CLIENT.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) {
                ...
                AndroidUtilities.runOnUIThread(() -> {
                    ...
                    completeShamalaWithResult(finalMessage, notify, scheduleDate, scheduleRepeatPeriod, payStars, internalParams);
                });
            }
        });
    }

    private void completeShamalaWithResult(CharSequence message, boolean notify, int scheduleDate, int scheduleRepeatPeriod, long payStars, SendMessageInternalParams internalParams) {
        shamalaTransformInProgress = false;
...
//        }
        ...
        float alpha = allowShare ? progress : 1f;
        sendButtonContainer.setAlpha(alpha);
        if (suggestButton != null) {
            suggestButton.setAlpha(suggestButton.getScaleX() > 0.7f ? alpha : 0);
        }
